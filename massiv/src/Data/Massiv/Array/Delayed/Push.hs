...
type Loader e =
  forall s. Scheduler s () -- ^ Scheduler to use for loading
...
-- @since 0.4.4
appendOuterM ::
     forall ix e m. (Index ix, Raises m)
  => Array DL ix e
  -> Array DL ix e
  -> m (Array DL ix e)
...
toLoadArray arr = DLArray (getComp arr) sz load
  where
    ...
    load :: Loader e
    load scheduler !startAt dlWrite dlSet =
      loadArrayWithSetST scheduler arr (dlWrite . (+ startAt)) (\offset -> dlSet (offset + startAt))
    {-# INLINE load #-}
...
fromStrideLoad stride arr =
  ...
  where
    ...
    load scheduler !startAt dlWrite _ =
      loadArrayWithStrideST scheduler stride newsz arr (\ !i -> dlWrite (i + startAt))
    {-# INLINE load #-}
...
instance Index ix => Load DL ix e where
  ...
  {-# INLINE replicate #-}
  loadArrayWithSetST scheduler DLArray {dlLoad} = dlLoad scheduler 0
  {-# INLINE loadArrayWithSetST #-}

instance Index ix => Functor (Array DL ix) where
  fmap f arr = arr {dlLoad = load f arr}
    where
      load g a scheduler startAt uWrite uSet =
        dlLoad a scheduler startAt (\ !i e -> uWrite i (g e)) (\o sz e -> uSet o sz (g e))
      {-# INLINE load #-}
  {-# INLINE fmap #-}
  (<$) e arr = arr {dlLoad = load e}
    where
      load :: a -> Loader a
      load x _ !startAt _ dlSet = dlSet startAt (linearSize arr) x
      {-# INLINE load #-}
  {-# INLINE (<$) #-}
