...
benchV1
  :: forall r e
   . (Typeable r, Typeable e, Source r e, Floating e, Numeric r e)
  => Vector r e
  -> Benchmark
benchV1 v =
  bgroup
    "normL2"
    [ bgroup
        (showsType @(Vector r e) (" - (" ++ show (size v) ++ ")"))
        [ bench "Seq" $ whnf normL2 v
        , bench "Par" $ whnf (normL2 . setComp Par) v
        ]
    , bgroup
        (showsType @(Vector D e) (" - (" ++ show (size v) ++ ")"))
...
benchVxV
  :: forall r e
   . (Typeable r, Typeable e, Numeric r e, Manifest r e)
  => VxV r e
  -> Benchmark
benchVxV vxv@VxV{..} =
  bgroup
    "dotProduct"
    [ bgroup
        (showsType @(VxV r e) (" - (" ++ showSizeVxV vxv ++ ")"))
        [ bench "Seq" $ whnf (aVxV !.!) bVxV
        , bench "Par" $ whnf (setComp Par aVxV !.!) bVxV
        ]
    , bgroup
        (showsType @(VxV D e) (" - (" ++ showSizeVxV vxv ++ ")"))
